...
with open(nameofRiver + "JustDeck.g01","w") as f:
    ...
    for i in range(len(stations)-1,0,-1):
        ...
        text = "Node Last Edited Time=May.08.2023 11:30:53\n"
        f.write(text)
        ...
        low = elevations[i]
        stat = bankStationLength + culvertThickness + topWidth
        text = "{:8.3f}{:8.3f}{:8.3f}{:8.3f}{:8.3f}{:8.3f}{:8.3f}{:8.3f}{:8.3f}{:8.3f}\n".format(0,high,bankStationLength + culvertThickness,high,bankStationLength + culvertThickness,low,stat,low,stat,high)
        f.write(text)

        text = "{:8.3f}{:8.3f}\n".format(stat + bankStationLength + culvertThickness,high)
        f.write(text)

        text = "#Mann= 3 ,0,0\n"
        f.write(text)
        
        text = "{:8.3f}{:8.3f}{:8.3f}{:8.3f}{:8.3f}{:8.3f}{:8.3f}{:8.3f}{:8.3f}\n".format(0,n,0,bankStationLength +culvertThickness,n,0,stat,n,0)
        f.write(text)
        
        text = "Bank Sta={:8.3f},{:8.3f}\n".format(bankStationLength + culvertThickness, stat)
        f.write(text)
        ...
        if stations[i] == 0:
            ...
            text = "\nType RM Length L Ch R = 3 ," + str(round(stations[i] - 0.01,3)) + "  ,,,\n"
            f.write(text)
            ...
            text = "{:8.3f}{:8.3f}\n".format(0 , 2*bankStationLength + 2*culvertThickness + topWidth)
            f.write(text)

            text = "{:8.3f}{:8.3f}\n".format(total + elevations[i],total + elevations[i])
            f.write(text)

            text = "{:8.3f}{:8.3f}\n".format(total + elevations[i] - 0.5, total + elevations[i] - 0.5)
            f.write(text)
            ...
            text = "{:8.3f}{:8.3f}\n".format(total + elevations[i-1] - 0.5, total + elevations[i-1] - 0.5)
            f.write(text)
