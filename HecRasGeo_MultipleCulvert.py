...
with open(nameofRiver + "MultipleCulvert.g01","w") as f:
    ...
    for i in range(len(stations)-1,0,-1):
        ...
        text = "0     .02       0       {:8.3f} .02       0     {:8.3f} .02       0\n".format(bankStationLength, stat)
        f.write(text)
        ...
        if (stations[i]-stations[i-1] > 0.11):
            ...
            text = "\nType RM Length L Ch R = 2 ," + str(round(stations[i] - 0.01,3)) + "  ,,,\n"
            f.write(text)
            ...
            text = "Multiple Barrel Culv=2,{},{},{},0.02,0.2,0.5,57,1,{},{}, 2,Culvert #1  , 0 ,0.01\n".format(total - 0.5,culvertTopWidth,round(stations[i]-stations[i-1]-0.02,3),elevations[i],elevations[i-1])
            f.write(text)

            text = "{:8.3f}{:8.3f}{:8.3f}{:8.3f}\n".format(bankStationLength + culvertThickness + culvertTopWidth/2,bankStationLength + culvertThickness + culvertTopWidth/2,
                                                           bankStationLength + culvertThickness + 0.3 + 3*culvertTopWidth/2,bankStationLength + culvertThickness + 0.3 + 3*culvertTopWidth/2)
            f.write(text)

            text = "BC Culvert Barrel=1,1,0\n"
            f.write(text)

            text = "BC Culvert Barrel=2,1,0\n"
            f.write(text)
